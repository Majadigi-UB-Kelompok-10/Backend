...
var (
	bansosSuffixFormat = regexp.MustCompile(`-[a-f0-9]{8}$`)
	bansosSlugFormat   = regexp.MustCompile(`^[a-z0-9]+(-[a-z0-9]+)*$`)
)

func TestGenerateSlug_OutputFormat(t *testing.T) {
	tests := []string{
		"Penerima Bansos Tahun 2026",
	...
	for _, title := range tests {
		t.Run(title, func(t *testing.T) {
			...
			if !bansosSlugFormat.MatchString(slug) {
			...
			if !bansosSuffixFormat.MatchString(slug) {
...
func TestGenerateSlug_FallbackForEmpty(t *testing.T) {
	for _, title := range []string{"", "   ", "!!!", "🎉🎉"} {
		t.Run(title, func(t *testing.T) {
			slug := GenerateSlug(title)
			if !strings.HasPrefix(slug, "bansos-") {
				t.Errorf("GenerateSlug(%q) = %q should start with 'bansos-'", title, slug)
			}
		})
	}
}
