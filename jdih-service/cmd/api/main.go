...
func loadConfig() (*Config, error) {
	...
	if cfg.CloudinaryURL == "" {
		slog.Warn("CLOUDINARY_URL kosong, fitur upload PDF mungkin tidak berfungsi")
	}
...
// =============================================================================
// SECURITY — credential masking dengan multi-pattern
// =============================================================================
var (
...
// =============================================================================
// CACHE (Mengembalikan instance interface agar bisa di-inject)
// =============================================================================
func initializeCache(cfg *Config) (cache.Cache, error) {
	if cfg.RedisURL == "" {
		slog.Warn("REDIS_URL kosong, menggunakan SimpleCache (in-memory) fallback")
		// Memakai fungsi NewSimpleCache dari cache.go milik JDIH
		return cache.NewSimpleCache(10000, 5*time.Minute), nil
	}
...
func initCloudinary(cfg *Config) (*cloudinary.Cloudinary, error) {
	if cfg.CloudinaryURL == "" {
		return nil, nil // Return nil, akan ditangkap validasi di handler jika mencoba upload
	}
...
func main() {
	...
	appCache, err := initializeCache(cfg)
	if err != nil {
	...
	if err != nil {
		slog.Error("init cloudinary failed", slog.String("error", maskSensitiveData(err.Error())))
	}
	...
	// Jalankan Cache Warmup di background
	...
	gracefulShutdown(app, pool, appCache, cfg)
}
...
func globalErrorHandler(cfg *Config) fiber.ErrorHandler {
	return func(c fiber.Ctx, err error) error {
		...
		message := "Terjadi kesalahan pada server JDIH"
...
// HEALTH ENDPOINTS
// =============================================================================
func registerHealthEndpoints(app *fiber.App, pool *pgxpool.Pool, appCache cache.Cache) {
...
// GRACEFUL SHUTDOWN
// =============================================================================
func gracefulShutdown(app *fiber.App, pool *pgxpool.Pool, appCache cache.Cache, cfg *Config) {
	...
	if rc, ok := appCache.(*cache.RedisCache); ok {
