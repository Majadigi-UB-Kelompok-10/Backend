...
var (
	jdihSuffixFormat = regexp.MustCompile(`-[a-f0-9]{8}$`)
	jdihSlugFormat   = regexp.MustCompile(`^[a-z0-9]+(-[a-z0-9]+)*$`)
)

func TestGenerateSlug_OutputFormat(t *testing.T) {
	tests := []string{
		"Peraturan Daerah No. 5 Tahun 2026",
		"Keputusan Gubernur: Tentang Pengadaan!",
	...
	for _, title := range tests {
		t.Run(title, func(t *testing.T) {
			...
			if !jdihSlugFormat.MatchString(slug) {
			...
			if !jdihSuffixFormat.MatchString(slug) {
...
func TestGenerateSlug_FallbackForEmpty(t *testing.T) {
	for _, title := range []string{"", "   ", "!!!", "⚖️⚖️"} {
		t.Run(title, func(t *testing.T) {
			slug := GenerateSlug(title)
			if !strings.HasPrefix(slug, "dokumen-") {
				t.Errorf("GenerateSlug(%q) = %q should start with 'dokumen-'", title, slug)
			}
		})
	}
}
