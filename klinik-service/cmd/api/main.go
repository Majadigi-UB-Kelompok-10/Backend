...
func initializeCache() {
	...
	if redisURL != "" {
		...
		slog.Warn("REDIS_URL tidak ditemukan, menggunakan SimpleCache (In-Memory Fallback)")
	}
}
...
func main() {
	...
	if err := godotenv.Load(); err != nil {
		slog.Info("File .env tidak ditemukan, menggunakan environment OS")
	}
	...
	app.Use(limiter.New(limiter.Config{
		...
		LimitReached: func(c fiber.Ctx) error {
			return c.Status(429).JSON(handlers.ErrorResponse{
				Code:    "ERR_RATE_LIMIT",
	...
	slog.Info("Menutup koneksi PostgreSQL...")
	pool.Close()
	...
	slog.Info("Proses shutdown selesai dengan aman.")
}