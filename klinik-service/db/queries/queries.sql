...
WHERE r.ticket_number = sqlc.arg('ticket_number')::text
LIMIT 1;
...
FROM hoax_reports
WHERE 
    sqlc.narg('status_filter')::report_status IS NULL 
    OR status = sqlc.narg('status_filter')::report_status
