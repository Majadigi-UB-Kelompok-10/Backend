...
func TestValidateSlug(t *testing.T) {
	tests := []struct {
		...
		{"valid simple", "pantai-balekambang", false},
		{"valid with numbers", "hotel-bintang-5", false},
		{"valid single word", "malang", false},
		{"valid with hex suffix", "coban-rondo-a1b2c3d4", false},

		{"empty", "", true},
		{"with uppercase", "Pantai-Balekambang", true},
		{"with spaces", "pantai balekambang", true},
		{"with special chars", "pantai!balekambang", true},
		{"starts with hyphen", "-pantai", true},
		{"ends with hyphen", "pantai-", true},
		{"double hyphen", "pantai--balekambang", true},
		{"only hyphen", "-", true},
	}
...
func TestValidateQueryString(t *testing.T) {
	tests := []struct {
		...
		{"valid simple", "kota malang", 100, false},
		{"valid with hyphen", "pantai-balekambang", 100, false},
		{"valid with dot", "kab.malang", 100, false},
		{"valid with underscore", "kota_batu", 100, false},
		{"valid with slash", "malang/raya", 100, false},
		{"valid with numbers", "hotel123", 100, false},
		{"empty (allowed)", "", 100, false},

		{"too long", "aaaaaaaaaaaaaaaaaaaaa", 10, true},
		{"only special chars stripped", "!@#$%^", 100, true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := ValidateQueryString(tt.input, tt.max, "test_field")
			if tt.wantErr && err == nil {
				t.Errorf("ValidateQueryString(%q) expected error, got nil", tt.input)
			}
			if !tt.wantErr && err != nil {
				t.Errorf("ValidateQueryString(%q) unexpected error: %v", tt.input, err)
			}
		})
	}
}
...
func TestValidateTextContent(t *testing.T) {
	tests := []struct {
		...
		{"valid simple", "Deskripsi destinasi wisata", 500, false},
		{"valid with newline", "Line 1\nLine 2", 500, false},
		{"trimmed", "  hello  ", 500, false},

		{"empty", "", 500, true},
		{"only spaces", "   ", 500, true},
		{"too long", "abcdefghijk", 5, true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			_, err := ValidateTextContent(tt.input, tt.max, "deskripsi")
			if tt.wantErr && err == nil {
				t.Errorf("ValidateTextContent(%q) expected error, got nil", tt.input)
			}
			if !tt.wantErr && err != nil {
				t.Errorf("ValidateTextContent(%q) unexpected error: %v", tt.input, err)
			}
		})
	}
}
...
func BenchmarkValidateSlug(b *testing.B) {
	for i := 0; i < b.N; i++ {
		_, _ = ValidateSlug("pantai-balekambang-a1b2c3d4", "slug")
	}
}
