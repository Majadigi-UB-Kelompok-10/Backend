...
func loadConfig() (*Config, error) {
	cfg := &Config{
		...
		ServicePublic: getEnv("SERVICE_PUBLIC_URL", "http://sinaker-api:8080/api/v1"),

		AllowedOrigins: parseList(getEnv("ALLOWED_ORIGINS",
			"http://localhost:3000,http://localhost:4000,http://localhost:5173")),
		BodyLimitBytes: getEnvInt("BODY_LIMIT_BYTES", 4*1024*1024), // 4MB
...
func globalErrorHandler(cfg *Config) fiber.ErrorHandler {
	return func(c fiber.Ctx, err error) error {
		...
		message := "Terjadi kesalahan pada server Sinaker"
