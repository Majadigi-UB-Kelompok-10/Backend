...
var (
	sinakerSuffixFormat = regexp.MustCompile(`-[a-f0-9]{8}$`)
	sinakerSlugFormat   = regexp.MustCompile(`^[a-z0-9]+(-[a-z0-9]+)*$`)
)

func TestGenerateSlug_OutputFormat(t *testing.T) {
	tests := []string{
		"Pelatihan Kerja Batch 2026",
		"Program BLK: Komputer & Desain!",
	...
	for _, title := range tests {
		t.Run(title, func(t *testing.T) {
			...
			if !sinakerSlugFormat.MatchString(slug) {
			...
			if !sinakerSuffixFormat.MatchString(slug) {
...
func TestGenerateSlug_FallbackForEmpty(t *testing.T) {
	for _, title := range []string{"", "   ", "!!!", "🎓🎓"} {
		t.Run(title, func(t *testing.T) {
			slug := GenerateSlug(title)
			if !strings.HasPrefix(slug, "blk-") {
				t.Errorf("GenerateSlug(%q) = %q should start with 'blk-'", title, slug)
			}
		})
	}
}
