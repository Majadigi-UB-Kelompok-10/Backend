...
var (
	siskaperbapSuffixFormat = regexp.MustCompile(`-[a-f0-9]{8}$`)
	siskaperbapoSlugFormat  = regexp.MustCompile(`^[a-z0-9]+(-[a-z0-9]+)*$`)
)

func TestGenerateSlug_OutputFormat(t *testing.T) {
	tests := []string{
		"Harga Beras Medium 2026",
		"Minyak Goreng: Update & Terbaru!",
	...
	for _, title := range tests {
		t.Run(title, func(t *testing.T) {
			...
			if !siskaperbapoSlugFormat.MatchString(slug) {
			...
			if !siskaperbapSuffixFormat.MatchString(slug) {
...
func TestGenerateSlug_FallbackForEmpty(t *testing.T) {
	for _, title := range []string{"", "   ", "!!!", "🛒🛒"} {
		t.Run(title, func(t *testing.T) {
			slug := GenerateSlug(title)
			if !strings.HasPrefix(slug, "bahan-pokok-") {
				t.Errorf("GenerateSlug(%q) = %q should start with 'bahan-pokok-'", title, slug)
			}
		})
	}
}
