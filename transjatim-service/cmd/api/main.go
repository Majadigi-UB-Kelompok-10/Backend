...
var (
	serviceName = "transjatim-service"
	version     = "dev"
	commit      = "unknown"
	buildTime   = "unknown"
	startedAt   = time.Now()
...
func loadConfig() (*Config, error) {
	cfg := &Config{
		...
		ServicePublic: getEnv("SERVICE_PUBLIC_URL", "http://transjatim-api:8080/api/v1"),
...
func globalErrorHandler(cfg *Config) fiber.ErrorHandler {
	return func(c fiber.Ctx, err error) error {
		...
		message := "Terjadi kesalahan pada server Trans Jatim"
